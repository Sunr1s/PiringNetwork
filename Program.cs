...
namespace TestNetwork
{
    class Program
    {
        ...
        public class Node
        {
            ...
            {
                var new_node = KeyReciverMixer(node.PublicKeys.GetValueOrDefault(address), node);
        ...
        public static async Task handelServer(Node node)
        {
            ...
            while (true)
            {
                ...
                await handleConnection(tcpClient, node);
        ...
        public static Task handleChipherConnection(SecretPackage deserializedWeatherForecast, Node node)
        {
            ...
            using (Aes aes = new AesCryptoServiceProvider())
            {
                if (node.mixedKey == null)
                    node = node.KeyMixer(node.PublicKeys.GetValueOrDefault(deserializedWeatherForecast.From), node);
